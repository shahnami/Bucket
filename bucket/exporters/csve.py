                ...
                if winner:
                    ...
                    if(winner.name == "AWS Collection"):
                        writer.writerow([page.domain, ", ".join([str(ip) for ip in page.ip]), page.header, page.title, page.status,
                                         page.redirect['location'], winner.name, ", ".join(winner.get_match_for(partial_page=winner.pages[page.domain])), page.ssl['valid']])
                    else:
                        writer.writerow([page.domain, ", ".join([str(ip) for ip in page.ip]), page.header, page.title, page.status, page.redirect['location'],
                                         winner.name, ", ".join([matched for matched in winner.pages[page.domain]['matched'] if matched in winner.keywords]), page.ssl['valid']])
                else:
